...
# this is a plain `cargo build`, a copy and a signature (`tools/teetotum-pack sign`, which runs
# under stable Rust). Inside this repository the build runs on the firmware's toolchain
# (`channel = "esp"`), whose `[unstable] build-std` builds `core` for the target as well; a face
# outside it needs only a stock `rustup target add wasm32v1-none`.
#
