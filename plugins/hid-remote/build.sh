...
# this is a plain `cargo build`, a copy and a signature (`tools/sign-face.py`). Inside this
# repository it runs on the firmware's toolchain (`channel = "esp"`), whose `[unstable] build-std`
# builds `core` for the target as well; a face outside it needs only a stock
