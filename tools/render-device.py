...
# How the height divides between knob and base is estimated.
KNOB_H = 11.5
...
USB_W, USB_H, USB_DEPTH = 9.0, 3.2, 1.6   # the Type-C socket in the base
...
def knurled_knob(name, radius, height, bottom, teeth=25, groove=0.5,
                 bevel=0.8, slant=45.0, dish=0.0, bore=None, mat=None):
    ...
    diagonal knurl). Four points per flute keep the wall round between the grooves.
    """
    spokes = teeth * 4
    ...
    for n, (zt, depth, ring_r) in enumerate(rings):
        ...
        for i in range(spokes):
            # Two points on the land, two in the groove.
            r = ring_r - (groove * depth if i % 4 in (2, 3) else 0.0)
            theta = i * step + twist
    ...
    # Mark the upright edges of the knurled rings sharp: an angle threshold would either facet
    # the chamfer or melt the flutes. On the face they would facet it into wedges.
    for edge in mesh.edges:
        a, b = edge.vertices
        if a % spokes == b % spokes and max(a, b) < knurled * spokes:
...
def build_device(art_path):
    ...
    base_z = BASE_H / 2
    cylinder("base", BASE_D / 2, BASE_H, base_z, bevel=BASE_BEVEL, mat=base_black)
    ...
    # The Type-C socket, a dark recess in the side of the base.
    bpy.ops.mesh.primitive_cube_add(size=1, location=(0, -BASE_D / 2 + USB_DEPTH / 2,
                                                     BASE_H * 0.45))
    sock = bpy.context.object
    sock.name = "usb"
    sock.scale = (USB_W / 2, USB_DEPTH / 2, USB_H / 2)
    sock.data.materials.append(dark)
...
CAMERAS = {
    ...
    # Shows wall, flutes and USB socket without flattening the glass into a narrow ellipse.
    "quarter": dict(elevation=40, azimuth=-125, distance=262, target=(0, 0, 14), lens=90),
}
